...
class IpRouteTable : TableQuery(OID(intArrayOf(1, 3, 6, 1, 2, 1, 4, 21))) {
    override fun getRowTitle(singleRow: MutableMap<String, QueryResponse>, i: Int): String {
        val ipRouteInfoTitle = singleRow["ipRouteInfo"]
        if (ipRouteInfoTitle != null) {
            val ipRouteInfo = ipRouteInfoTitle.value
            if (ipRouteInfo.isNotBlank()) {
                return "$i - $ipRouteInfo"
            }
        }
        return i.toString()
    }
